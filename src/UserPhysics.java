...
public class UserPhysics {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UserPhysics window = new UserPhysics();
	...
	public UserPhysics(JFrame frame) {
		this.frame = frame; 
	}
	...
	private void initialize() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new UserPhysics(listContesters());
			}
		});
...
private JFrame startExam() {	
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {	
					rs = s.executeQuery("select * from Physics where id = "+id+"");
					while(rs.next()) {
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(r == JOptionPane.YES_OPTION) {
					...
					try {
						s.execute("update contesters set physics_score = "+points+" where id = "+UserLogIn.getLogInfo()+"");						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					new UserPhysics().frame.setVisible(true);
					frame.dispose();
				}
			}
		});
