...
public class UserProgramming {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UserProgramming window = new UserProgramming();
	 ...
	 */
	public UserProgramming() {
		initialize();
	}

	public UserProgramming(JFrame f) {
		this.f = f; 
	}
	...
	private void initialize() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new UserProgramming(listContesters());
			}
		});
...
private JFrame startExam() {	
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {	
					rs = s.executeQuery("select * from Programming where id = "+id+"");
					while(rs.next()) {
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(r == JOptionPane.YES_OPTION) {
					...
					try {
						s.execute("update contesters set programming_score = "+points+" where id = "+UserLogIn.getLogInfo()+"");						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					new UserProgramming().frame.setVisible(true);
					frame.dispose();
				}
			}
		});
...
private JFrame listContesters() {
	...
	btnNewButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			new UserMath().frame.setVisible(true);
			frame.dispose();		
		}
	});
	frame.getContentPane().add(btnNewButton);
