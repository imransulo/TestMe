...
public class TeacherMath {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherMath window = new TeacherMath();
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 890, 665);
		...
		listQuestions.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherMath(questionsTable());			
		...
		addQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String query1 = "select question from Math";
		...
		editQuestion.setBounds(292, 269, 253, 48);
		...
		removeQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherMath(removeQuestion());
		...
		removeQuestion.setBounds(292, 338, 253, 48);
		...
		listContesters.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherMath(listContesters());
			}
		});
		...
		listContesters.setBounds(292, 409, 253, 48);
		...
		btnNewButton_1.setBounds(775, 580, 89, 35);
		...
		lblNewLabel.setBounds(21, 11, 796, 108);
		frame.getContentPane().add(lblNewLabel);
		
		JButton btnNewButton_2 = new JButton("Remove Contester");
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				frame.dispose();
				new TeacherMath(removeContester());
			}
		});
		btnNewButton_2.setForeground(new Color(255, 255, 255));
		btnNewButton_2.setBackground(new Color(255, 140, 0));
		btnNewButton_2.setFont(new Font("Tahoma", Font.PLAIN, 16));
		btnNewButton_2.setBounds(292, 478, 253, 48);
	...
	private JFrame questionsTable() {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
			Statement s = con.createStatement();
			String query1 = "Select Id,Question,correct_alternative as Answer from Math";
			ResultSet rs = s.executeQuery(query1);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {	
				...
				if(oldQuestion.getText().trim().isEmpty() || newQuestion.getText().trim().isEmpty())
				{
					...
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Math where id = '"+oldQuestion.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) { 
						
						String query2 = "update Math set question = '"+newQuestion.getText()+"' where id = '"+oldQuestion.getText()+"'";
						s.execute(query2); 
						
						if(!first.getText().trim().isEmpty()) {
						String query3 = "update Math set first_alternative = '"+first.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query3); 
						}						
						if(!second.getText().trim().isEmpty()) {
						String query4 = "update Math set second_alternative = '"+second.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query4);
						}
						if(!third.getText().trim().isEmpty()) {
						String query5 = "update Math set third_alternative = '"+third.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query5); 
						}
						if(!fourth.getText().trim().isEmpty()) {
						String query6 = "update Math set fourth_alternative = '"+fourth.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query6); 
						}
						if(!correct.getText().trim().isEmpty()) {
						String query7 = "update Math set correct_alternative = '"+correct.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query7); 
						}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Math where id = '"+question.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) {
						String query2 = "delete from Math where id = '"+question.getText()+"'";
						s.execute(query2);
						new TeacherMath().frame.setVisible(true);
						frame.dispose();
