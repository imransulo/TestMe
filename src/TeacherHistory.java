...
public class TeacherHistory {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherHistory window = new TeacherHistory();
	...
	public TeacherHistory(JFrame a) {	
		this.a = a;
	}
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 890, 675);
		...
		listQuestions.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherHistory(questionsTable());			
		...
		addQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String query1 = "select question from History";
				...
				if(limit == 20) JOptionPane.showMessageDialog(null, "               Maximum number of questions has been reached.\n Remove a question to add a new one or edit an existing question.");
					...
					frame.dispose();				
				    new TeacherHistory(addQuestion());	
				}
			}
		});
		...
		editQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherHistory(editQuestion());
		...
		removeQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherHistory(removeQuestion());
		...
		listContesters.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherHistory(listContesters());
			}
		});
		...
		btnNewButton_1.setBounds(775, 590, 89, 35);
		...
		listContesters_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherHistory(removeContester());				
			}
		});
	...
	private JFrame questionsTable() {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
			Statement s = con.createStatement();
			String query1 = "Select ID,Question,correct_alternative as Answer from History";
			ResultSet rs = s.executeQuery(query1);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "insert into History(question,first_alternative,second_alternative,third_alternative,fourth_alternative,correct_alternative) "
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {	
				...
				if(oldQuestion.getText().trim().isEmpty() || newQuestion.getText().trim().isEmpty())
				{
					...
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from History where id = '"+oldQuestion.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) { 
						
						String query2 = "update History set question = '"+newQuestion.getText()+"' where id = '"+oldQuestion.getText()+"'";
						s.execute(query2); 
						
						if(!first.getText().trim().isEmpty()) {
						String query3 = "update History set first_alternative = '"+first.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query3); 
						}						
						if(!second.getText().trim().isEmpty()) {
						String query4 = "update History set second_alternative = '"+second.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query4);
						}
						if(!third.getText().trim().isEmpty()) {
						String query5 = "update History set third_alternative = '"+third.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query5); 
						}
						if(!fourth.getText().trim().isEmpty()) {
						String query6 = "update History set fourth_alternative = '"+fourth.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query6); 
						}
						if(!correct.getText().trim().isEmpty()) {
						String query7 = "update History set correct_alternative = '"+correct.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query7); 
						}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from History where id = '"+question.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) {
						String query2 = "delete from History where id = '"+question.getText()+"'";
						s.execute(query2);
						new TeacherHistory().frame.setVisible(true);
						frame.dispose();
