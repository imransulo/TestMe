...
public class UserEconomy {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UserEconomy window = new UserEconomy();
	...
	public UserEconomy(JFrame frame) {
		this.frame = frame; 
	}
	...
	private void initialize() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new UserEconomy(listContesters());
			}
		});
  ...
  private JFrame startExam() {	
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {	
					rs = s.executeQuery("select * from Economy where id = "+id+"");
					while(rs.next()) {
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(r == JOptionPane.YES_OPTION) {
					...
					try {
						s.execute("update contesters set economy_score = "+points+" where id = "+UserLogIn.getLogInfo()+"");						
					} catch (SQLException e1) {
