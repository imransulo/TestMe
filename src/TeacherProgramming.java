...
public class TeacherProgramming {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherProgramming window = new TeacherProgramming();
	 ...
	 */
	public TeacherProgramming() {
		initialize();
	}

	public TeacherProgramming(JFrame a) {	
		this.a = a;
	}
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 890, 681);
		...
		listQuestions.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherProgramming(questionsTable());			
		...
		addQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String query1 = "select question from Programming";
				...
				if(limit == 20) JOptionPane.showMessageDialog(null, "               Maximum number of questions has been reached.\n Remove a question to add a new one or edit an existing question.");
					...
					frame.dispose();				
				    new TeacherProgramming(addQuestion());	
				}
			}
		});
		...
		editQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherProgramming(editQuestion());
		...
		removeQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherProgramming(removeQuestion());
		...
		listContesters.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherProgramming(listContesters()); 
			}
		});
		...
		btnNewButton_1.setBounds(775, 596, 89, 35);
		frame.getContentPane().add(btnNewButton_1);
		
		JLabel lblNewLabel = new JLabel("Programming Section");
		...
		btnRemoveContester.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherProgramming(removeContester()); 
			}
		});
		...
		btnRemoveContester.setBounds(292, 499, 253, 48);
	...
	private JFrame questionsTable() {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
			Statement s = con.createStatement();
			String query1 = "Select ID,Question,correct_alternative as Answer from Programming";
			ResultSet rs = s.executeQuery(query1);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "insert into Programming(question,first_alternative,second_alternative,third_alternative,fourth_alternative,correct_alternative) "
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {	
				...
				if(oldQuestion.getText().trim().isEmpty() || newQuestion.getText().trim().isEmpty())
				{
					...
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Programming where id = '"+oldQuestion.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) { 
						
						String query2 = "update Programming set question = '"+newQuestion.getText()+"' where id = '"+oldQuestion.getText()+"'";
						s.execute(query2); 
						
						if(!first.getText().trim().isEmpty()) {
						String query3 = "update Programming set first_alternative = '"+first.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query3); 
						}						
						if(!second.getText().trim().isEmpty()) {
						String query4 = "update Programming set second_alternative = '"+second.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query4);
						}
						if(!third.getText().trim().isEmpty()) {
						String query5 = "update Programming set third_alternative = '"+third.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query5); 
						}
						if(!fourth.getText().trim().isEmpty()) {
						String query6 = "update Programming set fourth_alternative = '"+fourth.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query6); 
						}
						if(!correct.getText().trim().isEmpty()) {
						String query7 = "update Programming set correct_alternative = '"+correct.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query7); 
						}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Programming where id = '"+question.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) {
						String query2 = "delete from Programming where id = '"+question.getText()+"'";
						s.execute(query2);
						new TeacherProgramming().frame.setVisible(true);
						frame.dispose();
