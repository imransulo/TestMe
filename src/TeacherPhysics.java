...
public class TeacherPhysics {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherPhysics window = new TeacherPhysics();
	 ...
	 */
	public TeacherPhysics(JFrame a) {	
		this.a = a;
	}
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 890, 659);
		...
		listQuestions.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherPhysics(questionsTable());			
		...
		listQuestions.setBounds(292, 130, 253, 48);
		...
		addQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String query1 = "select question from Physics";
				...
				if(limit == 20) JOptionPane.showMessageDialog(null, "               Maximum number of questions has been reached.\n Remove a question to add a new one or edit an existing question.");
					...
					frame.dispose();				
				    new TeacherPhysics(addQuestion());	
				}
			}
		});
		...
		addQuestion.setBounds(292, 199, 253, 48);
		...
		editQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherPhysics(editQuestion());
		...
		editQuestion.setBounds(292, 267, 253, 48);
		...
		removeQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherPhysics(removeQuestion());
		...
		removeQuestion.setBounds(292, 335, 253, 48);
		...
		listContesters.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new TeacherPhysics(listContesters());			
			}
		});
		...
		listContesters.setBounds(292, 404, 253, 48);
		...
		btnNewButton_1.setBounds(775, 574, 89, 35);
		frame.getContentPane().add(btnNewButton_1);
		
		JLabel lblNewLabel = new JLabel("Physics Section");
		lblNewLabel.setForeground(new Color(255, 255, 255));
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 50));
		lblNewLabel.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel.setBounds(-11, 0, 854, 108);
		frame.getContentPane().add(lblNewLabel);
		
		JButton btnRemoveContester = new JButton("Remove Contester");
		btnRemoveContester.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				frame.dispose();
				new TeacherPhysics(removeContester());
			}
		});
		btnRemoveContester.setForeground(Color.WHITE);
		btnRemoveContester.setFont(new Font("Tahoma", Font.PLAIN, 16));
		btnRemoveContester.setBackground(new Color(255, 140, 0));
		btnRemoveContester.setBounds(292, 473, 253, 48);
		frame.getContentPane().add(btnRemoveContester);
	}
	...
	private JFrame questionsTable() {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
			Statement s = con.createStatement();
			String query1 = "Select ID,Question,correct_alternative as Answer from Physics";
			ResultSet rs = s.executeQuery(query1);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "insert into Physics(question,first_alternative,second_alternative,third_alternative,fourth_alternative,correct_alternative) "
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {	
				...
				if(oldQuestion.getText().trim().isEmpty() || newQuestion.getText().trim().isEmpty())
				{
					...
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Physics where id = '"+oldQuestion.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) { 
						
						String query2 = "update Physics set question = '"+newQuestion.getText()+"' where id = '"+oldQuestion.getText()+"'";
						s.execute(query2); 
						
						if(!first.getText().trim().isEmpty()) {
						String query3 = "update Physics set first_alternative = '"+first.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query3); 
						}						
						if(!second.getText().trim().isEmpty()) {
						String query4 = "update Physics set second_alternative = '"+second.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query4);
						}
						if(!third.getText().trim().isEmpty()) {
						String query5 = "update Physics set third_alternative = '"+third.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query5); 
						}
						if(!fourth.getText().trim().isEmpty()) {
						String query6 = "update Physics set fourth_alternative = '"+fourth.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query6); 
						}
						if(!correct.getText().trim().isEmpty()) {
						String query7 = "update Physics set correct_alternative = '"+correct.getText()+"' where question = '"+newQuestion.getText()+"'";
						s.execute(query7); 
						}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/testme","root","root");
					Statement s = con.createStatement();
					String query1 = "Select question from Physics where id = '"+question.getText()+"'";
					ResultSet rs = s.executeQuery(query1);
					if(rs.next()) {
						String query2 = "delete from Physics where id = '"+question.getText()+"'";
						s.execute(query2);
						new TeacherPhysics().frame.setVisible(true);
						frame.dispose();
