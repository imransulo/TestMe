...
public class UserChemistry {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UserChemistry window = new UserChemistry();
	 ...
	 */
	public UserChemistry() {
		initialize();
	}

	public UserChemistry(JFrame frame) {
		this.frame = frame; 
	}
	...
	private void initialize() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				new UserChemistry(listContesters());
			}
		});
...
private JFrame startExam() {	
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {	
					rs = s.executeQuery("select * from Chemistry where id = "+id+"");
					while(rs.next()) {
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(r == JOptionPane.YES_OPTION) {
					...
					try {
						s.execute("update contesters set chemistry_score = "+points+" where id = "+UserLogIn.getLogInfo()+"");						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					new UserChemistry().frame.setVisible(true);
					frame.dispose();
				}
			}
		});
